...
dfDiceJobs2 <-
  dfDiceJobs %>%
  dplyr::mutate( AdvertiserUrl = NULL, SiteName = NULL, Description = NULL, Skills = NULL,
                 Shift = NULL)
```
...
```{r}
lsFocusColumns <- setdiff( colnames(dfDiceJobs2), c( "ID", "AdvertiserUrl", "SiteName", "Description", "Skills", "Shift" ) )
lsSMats <- 
  purrr::map( lsFocusColumns, function(x) {
    dfX <- dfDiceJobs2[,c("ID", x)]
...
smrDiceJobs %>% 
  ...
  SMRMonJoinAcross( data = dfDiceJobs2, by = "ID" ) %>% 
  SMRMonTakeValue
```

...
if( FALSE ) {
  ...
  load( file = file.path( savedObjectsDirName, paste0( modelID, "-", "lsTaxonomyTagsLSAObjects.RData" ) ) )
  
} else {
  
  load( file = "lsTaxonomyTagsRecommenders.RData" )
  load( file = "lsTaxonomyTagsLSAObjects.RData" )
...
```{r}
selectInput( inputId = "taxonomyTagsRecommender", label = "Taxonomy tags recommender:", choices = names(lsTaxonomyTagsRecommenders), selected = "Title")
...
lsaObj <- 
  reactive({
    lsTaxonomyTagsLSAObjects[[ input$taxonomyTagsRecommender ]]
  })
...
```{r}
textAreaInput( inputId = "tagsText", label = "Tag text (metadata)", placeholder = "Skill:clojure, Title:quant", width = "600px", cols = 300, rows = 8 )
...
```{r}
textAreaInput( inputId = "freeText", label = "Free text (description)", placeholder = "PhD in physics, statistics, mathematics, operations research, or similar.", width = "600px", cols = 300, rows = 8 )
